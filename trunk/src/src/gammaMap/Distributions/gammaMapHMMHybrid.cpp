...
#include <gammaMap/Utilities/gammaMapUtils.h>
#include <gsl/gsl_sf_hyperg.h>
#include <gammaMap/Utilities/mutation.h>
...
const string gammaMapHMMHybridParameterNames[10] = {"anc","theta","kappa","gamma1","gamma2","T","p","pi","gamma1_wt","gamma2_wt"};

gammaMapHMMHybrid::gammaMapHMMHybrid(const int L, const int ng1, const int ng2, string name, DAG* dag) : DAGcomponent(name,dag,"gammaMapHMMHybrid"), Distribution(gammaMapHMMHybridParameterNames,10), _L(L), _ng1(ng1), _ng2(ng2), _anc_changed(true),
...
/*gammaMapHMMHybrid::gammaMapHMMHybrid(const gammaMapHMMHybrid& x) : DAGcomponent(x), Distribution(x), _anc_changed(x._anc_changed),
...
bool gammaMapHMMHybrid::check_random_variable_type(RandomVariable* random_variable) {
...
bool gammaMapHMMHybrid::check_parameter_type(const int i, Variable* parameter) {
	switch(i) {
		case 0:	{	//	anc
			// Insist on the particular RandomVariable (not just Variable) as it also specifies the encoding
			...
			if(!(rv==0) && rv->length()!=seqlen()) {
				stringstream errMsg;
				errMsg << "gammaMapHMMHybrid::check_parameter_type(): Codon61SequenceRV object " << rv->name() << " has sequence length " << rv->length();
				errMsg << " whereas gammaMapHMMHybrid object " << name() << " has sequence length " << seqlen();
		...
		case 3:	{	//	gamma1
			...
			if(!(rv==0) && rv->length()!=ngamma1()) {
				stringstream errMsg;
				errMsg << "gammaMapHMMHybrid::check_parameter_type(): ContinuousVectorVariable object " << rv->name() << " has " << rv->length();
				errMsg << " entries whereas gammaMapHMMHybrid object " << name() << " requires " << ngamma1();
				error(errMsg.str().c_str());
			}
			return rv;
		}
		case 4:	{	//	gamma2
			...
			if(!(rv==0) && rv->length()!=ngamma2()) {
				stringstream errMsg;
				errMsg << "gammaMapHMMHybrid::check_parameter_type(): ContinuousVectorVariable object " << rv->name() << " has " << rv->length();
				errMsg << " entries whereas gammaMapHMMHybrid object " << name() << " requires " << ngamma2();
				error(errMsg.str().c_str());
			}
			return rv;
		}
		...
		default:
			error("gammaMapHMMHybrid::check_parameter_type(): parameter not found");
	}
	return false;
}

int gammaMapHMMHybrid::seqlen() const {
	return _L;
}

int gammaMapHMMHybrid::ngamma1() const {
	return _ng1;
}

int gammaMapHMMHybrid::ngamma2() const {
	return _ng2;
}

void gammaMapHMMHybrid::set_anc(Codon61SequenceRV* anc) {
	set_parameter(0,(Variable*)anc);
}

void gammaMapHMMHybrid::set_theta(ContinuousVariable* theta) {
	set_parameter(1,(Variable*)theta);
}

void gammaMapHMMHybrid::set_kappa(ContinuousVariable* kappa) {
	set_parameter(2,(Variable*)kappa);
}

void gammaMapHMMHybrid::set_gamma1(ContinuousVectorVariable* gamma) {
	set_parameter(3,(Variable*)gamma);
}

void gammaMapHMMHybrid::set_gamma2(ContinuousVectorVariable* gamma) {
	set_parameter(4,(Variable*)gamma);
}

void gammaMapHMMHybrid::set_T(ContinuousVariable* T) {
	set_parameter(5,(Variable*)T);
}

void gammaMapHMMHybrid::set_p(ContinuousVariable* p) {
	set_parameter(6,(Variable*)p);
}

void gammaMapHMMHybrid::set_pi(ContinuousVectorVariable* pi) {
	set_parameter(7,(Variable*)pi);
}

void gammaMapHMMHybrid::set_gamma1_wt(ContinuousVectorVariable* gamma_wt) {
	set_parameter(8,(Variable*)gamma_wt);
}

void gammaMapHMMHybrid::set_gamma2_wt(ContinuousVectorVariable* gamma_wt) {
	set_parameter(9,(Variable*)gamma_wt);
}

const Codon61SequenceRV* gammaMapHMMHybrid::get_anc() const {
	return (const Codon61SequenceRV*)get_parameter(0);
}

const ContinuousVariable* gammaMapHMMHybrid::get_theta() const {
	return (const ContinuousVariable*)get_parameter(1);
}

const ContinuousVariable* gammaMapHMMHybrid::get_kappa() const {
	return (const ContinuousVariable*)get_parameter(2);
}

const ContinuousVectorVariable* gammaMapHMMHybrid::get_gamma1() const {
	return (const ContinuousVectorVariable*)get_parameter(3);
}

const ContinuousVectorVariable* gammaMapHMMHybrid::get_gamma2() const {
	return (const ContinuousVectorVariable*)get_parameter(4);
}

const ContinuousVariable* gammaMapHMMHybrid::get_T() const {
	return (const ContinuousVariable*)get_parameter(5);
}

const ContinuousVariable* gammaMapHMMHybrid::get_p() const {
	return (const ContinuousVariable*)get_parameter(6);
}

const ContinuousVectorVariable* gammaMapHMMHybrid::get_pi() const {
	return (const ContinuousVectorVariable*)get_parameter(7);
}

const ContinuousVectorVariable* gammaMapHMMHybrid::get_gamma1_wt() const {
	return (const ContinuousVectorVariable*)get_parameter(8);
}

const ContinuousVectorVariable* gammaMapHMMHybrid::get_gamma2_wt() const {
	return (const ContinuousVectorVariable*)get_parameter(9);
}

mydouble gammaMapHMMHybrid::likelihood(const RandomVariable* rv, const Value* val) {
	// Remember: there may be multiple child RVs!
	map< const RandomVariable*, int >::iterator it = _rv_index.find(rv);
	if(it==_rv_index.end()) error("gammaMapHMMHybrid::likelihood(): RandomVariable not recognised");
...
void gammaMapHMMHybrid::add_random_variable(RandomVariable* random_variable) {
	...
	if(rv->length()!=seqlen()) {
		stringstream errMsg;
		errMsg << "gammaMapHMMHybrid::add_random_variable(): Codon61Count object " << rv->name() << " has sequence length " << rv->length();
		errMsg << " whereas gammaMapHMMHybrid object " << name() << " has sequence length " << seqlen();
		error(errMsg.str().c_str());
	}
	// Memory allocation
...
void gammaMapHMMHybrid::remove_random_variable(RandomVariable* random_variable) {
	error("gammaMapHMMHybrid::remove_random_variable(): not permitted");
}

void gammaMapHMMHybrid::receive_signal_from_parent(const Value* v, const Variable::Signal sgl) {
...
void gammaMapHMMHybrid::update_phylo_rate_matrix() {
	...
	void gammaMapHMMHybrid::update_popgen_rate_matrix() {
		...
		void gammaMapHMMHybrid::update_pclik(const Value* val, const int ix) {
...
#ifdef _DEBUG_OMEGAMAP_NCD_HMM
				{
					...
					if(full_recalc || _anc.has_changed(pos)) {
						...
						for(anc=0;anc<61;anc++) {
							...
							const char aAA = gammaMapUtils::oneLetterCodes[anc];
							...
							for(i=0;i<61;i++) {
								...
								ttheta += muti;
								if(gammaMapUtils::oneLetterCodes[i]==aAA) {
									pt1 += cti;
									ptheta += muti;
								}
							...
							for(g=0;g<_ng1;g++) {
								...
								if(fabs(sigma)<1e-3) {
									...
									const double hpnum = (gammaMapUtils::hypergeometric1F1(pt1+ptheta,Npos+ttheta,-sigma)-1.0);
									const double hpden = (gammaMapUtils::hypergeometric1F1(ptheta,ttheta,-sigma)-1.0);
			...
			void gammaMapHMMHybrid::update_pemiss(const int ix) {
				...
				void gammaMapHMMHybrid::update_alphabeta(const int ix) {
				...
				int gammaMapHMMHybrid::recentre() const {
					return (_L-1)/2;
				}
				
				void gammaMapHMMHybrid::forward(const int ix, const int left) {
				...
				void gammaMapHMMHybrid::backward(const int ix, const int right) {
				...
				mydouble gammaMapHMMHybrid::HMMlik(const int ix) {
					mydouble lik = 0.0;
					int g;
					for(g=0;g<_ng1;g++) {
				...
				vector<double> gammaMapHMMHybrid::path_sampler(const RandomVariable* const rv) {
					...
					map< const RandomVariable*, int >::iterator it = _rv_index.find(rv);
					if(it==_rv_index.end()) error("gammaMapHMMHybrid::path_sampler(): RandomVariable not recognised");
					const int ix = it->second;
					...
					if(g==_ng1) error("gammaMapHMMHybrid::path_sampler(): did not draw path correctly");
					draw[pos] = g;
...
//						ret[pos] = final_draw(pos,g,_ran.U(),m,gamma1,gamma2,pg,qdraw,pemiss);
				...
				double gammaMapHMMHybrid::final_draw(const int pos, const int draw, const double U_in, const mydouble m, const vector<double> &gamma1, const vector<double> &gamma2, const vector<mydouble> &pg, vector<mydouble> qdraw, const Matrix<mydouble> &pemiss) {
					...
					if(h==_ng2+1) error("gammaMapHMMHybrid::final_draw(): did not draw path correctly");
					if(h==_ng2) return gamma1[draw];
					return gamma2[h];
				}
				
				const string gammaMapHMMHybridPathSamplerParameterNames[0];
				
				gammaMapHMMHybridPathSampler::gammaMapHMMHybridPathSampler(string rv_name, string distribution_name, string name, DAG* dag) : DAGcomponent(name,dag,"gammaMapHMMHybridPathSampler"), Transformation(gammaMapHMMHybridPathSamplerParameterNames,0), _distribution_name(distribution_name) {
					_rv = getDAG()->get_random_variable(rv_name);
				}
				
				gammaMapHMMHybridPathSampler::gammaMapHMMHybridPathSampler(const gammaMapHMMHybridPathSampler& x) : DAGcomponent(x), Transformation(x), _distribution_name(x._distribution_name), _hmm(x._hmm), _rv(x._rv), _L(x._L) {
				}
				
				string gammaMapHMMHybridPathSampler::validate() const {
					// For starters, do not allow this to parameterize anything else
					if(n_child_distributions()+n_child_transformations()>0) {
						string errTxt = "gammaMapHMMHybridPathSampler: object " + name() + " may not parameterize other objects";
						return errTxt;
					}
					...
					_hmm = dynamic_cast<gammaMapHMMHybrid*>(getDAG()->get_distribution(_distribution_name));
					if(!_hmm) {
						string errTxt = "gammaMapHMMHybridPathSampler: object " + name() + " cannot find gammaMapHMMHybrid object " + _distribution_name;
						return errTxt;
					}
					const Codon61Count* ct = dynamic_cast<const Codon61Count*>(_rv);
					if(!ct) {
						string errTxt = "gammaMapHMMHybridPathSampler: " + _rv->name() + " is not of type Codon61Count";
						return errTxt;
					}
					_L = ct->length();
					return "";
				...
				int gammaMapHMMHybridPathSampler::length() const {
					return _L;
				}
				...
				double gammaMapHMMHybridPathSampler::get_double(const int i) const {
					if(i<0) error("gammaMapHMMHybridPathSampler::get_double(): index cannot be negative");
					vector<double> x = get_doubles();
					if(i>=_L) error("gammaMapHMMHybridPathSampler::get_double(): index too large");
					return x[i];
				}
				
				vector<double> gammaMapHMMHybridPathSampler::get_doubles() const {
					return _hmm->path_sampler(_rv);
				}
				
				bool gammaMapHMMHybridPathSampler::has_changed(const int i) const {
					return true;
				}
				
				vector<bool> gammaMapHMMHybridPathSampler::has_changed() const {
					return vector<bool>(length(),true);
				}
				
				bool gammaMapHMMHybridPathSampler::check_parameter_type(const int i, Variable* parameter) {
					switch(i) {
						default:
							error("gammaMapHMMHybridPathSampler::check_parameter_type(): parameter not found");
					}
					return false;
				}
				
				void gammaMapHMMHybridPathSampler::print(ostream& out, string sep) {
					int i;
					vector<double> x = get_doubles();
					for(i=0;i<length();i++) {
						if(i>0) out << sep;
