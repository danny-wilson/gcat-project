 ...
 */
#ifndef _GAMMAMAP_HMM_HYBRID_H_
#define _GAMMAMAP_HMM_HYBRID_H_
...
class gammaMapHMMHybrid : public Distribution {
private:
	// Sequence length
	int _L;
...
public:
	// Constructor
	gammaMapHMMHybrid(const int L, const int ng1, const int ng2, string name="", DAG* dag=0);
	// Copy constructor
	//gammaMapHMMHybrid(const gammaMapHMMHybrid& x);
...
class gammaMapHMMHybridPathSampler : public ContinuousVectorVariable, public Transformation {
private:
	string _distribution_name;		// Initially store just the name, until validate() is called
	mutable gammaMapHMMHybrid* _hmm;		// Distribution
	RandomVariable* _rv;			// RV index wrt the distribution
	mutable int _L;					// Length
	
public:
	// Constructor
	gammaMapHMMHybridPathSampler(string rv_name, string distribution_name, string name="", DAG* dag=0);
	// Copy constructor
	gammaMapHMMHybridPathSampler(const gammaMapHMMHybridPathSampler& x);
